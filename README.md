...
- `src/pages/hw1/index.mdx`：HW1 的讲解、关键代码、展示三个标签内容。
...
新增作业时，先将其 `ready` 设为 `true`、更新标题和简介，再创建对应 `src/pages/hwN/index.mdx` 或 `index.astro`。占位路由会自动排除已完成作业，避免重复路径。大型可视化页面可直接使用 BaseLayout，不必使用文档布局。
...
每份作业的新会话先读 [作业开发与交接指南](docs/HOMEWORK-HANDOFF.md)，其中包含可直接复制的提示词、小节级三标签 MDX 模板与验证要求。当前作业状态见 [HW1 交接](docs/hw1.md)。
