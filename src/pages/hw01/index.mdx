...
下面的示意把一张卡片拆成 `margin → border → padding → content` 四层。由外向内观察：margin 位于边框之外，padding 留在边框与内容之间，content 是内容所在区域。各层仅表示包含关系，不按实际尺寸绘制。
...
“正常流”示例中的标题、正文和操作区依次占位；“局部定位”示例只有角标绝对定位在卡片右上角，正文仍按原顺序排列。
...
这个页面结构示例使用两列：页头和页脚跨越整行，侧栏占固定轨道，主内容中排列三张卡片。视口窄于 560px 时改为单列。
...
<div class="toolbar">
  ...
  <div class="toolbar-actions">
    <button type="button">自适应宽度</button>
    <button type="button">窄容器</button>
  </div>
</div>
```
...
示例把标题区和操作区放在同一条 Flex 轴上。点击“窄容器”缩小展示区后，`flex-wrap` 允许操作区换到下一行，内容不会因为强行保持一行而溢出；点击“自适应宽度”可以恢复展示区的可用宽度；空间充足时，标题与按钮回到同一行。
...
`@container` 查询的是 `.content-region` 的宽度。展示区里把预览宽度设为“窄容器”，容器宽度变为 `360px`，小于 `560px` 的断点，就能在不改变浏览器窗口的情况下触发这条规则。
...
先把侧栏和区域间距调大，再把预览宽度设为“窄容器”。注意观察：预览宽度收窄后，侧栏宽度不再参与左右分栏，页面改为上下流动，三张卡片也变为单列。
